...
const theatersTemplate = (theaters) => html `
<div id="welcomeMessage">
            ...
            <div id="events">
            <h1>Future Events</h1>
            <div class="theaters-container">${theaters.length ? theaters.map(theaterView) : html`<h4 class="no-event">No Events Yet...</h4>`}</div>
    </div>`;

const theaterView = (theater) => html`
...
export async function theatersPage(context) {
    const theaters = await getAllTheatres();
    console.log(theaters)
    context.render(theatersTemplate(theaters));
    context.setUserNav()
}